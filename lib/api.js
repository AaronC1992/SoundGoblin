...
function getBackendUrl() {
    ...
    if (typeof window !== 'undefined' && window.SoundGoblin_BACKEND_URL) {
        return window.SoundGoblin_BACKEND_URL;
    }
    ...
    try {
        ...
        if (resp.ok) {
            ...
            if (Array.isArray(data?.files)) {
                window.debugLog(`[SoundGoblin] Loaded ${data.files.length} sounds from local saved-sounds.json`);
...
/**
 * Analyze transcript via the SoundGoblin backend (requires active subscription token)
...
async function analyzeTranscript(payload) {
    ...
    let token = null;
    try { token = localStorage.getItem('SoundGoblin_token'); } catch (_) {}

    const headers = { 'Content-Type': 'application/json' };
    if (token) headers['Authorization'] = `Bearer ${token}`;

    return await apiFetchJson('/analyze', {
        method: 'POST',
        headers,
