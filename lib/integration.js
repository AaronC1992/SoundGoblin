...
export function initEnhancements() {
    if (CONFIG.DEBUG_MODE) console.log('[SoundGoblin] Initializing enhancements v3.0.0...');
    ...
    globals.memoryMonitor.onCritical = (memMB) => {
        ...
        window.dispatchEvent(new CustomEvent('soundgoblin:memory-critical', { detail: { memoryUsageMB: memMB } }));
    };
    ...
    try {
        ...
        window.SoundGoblin_Accessibility = a11y;
        
        // Set up keyboard event listeners
        setupKeyboardEvents();
    } catch (e) {
        console.warn('[SoundGoblin] Accessibility initialization failed:', e);
    }
    ...
    if (CONFIG.DEBUG_MODE) {
        window.SoundGoblin_Globals = globals;
        window.SoundGoblin_PerformanceReport = () => console.table(globals.performanceMonitor.generateReport());
        window.SoundGoblin_MemoryStats = () => console.table(globals.memoryMonitor.getStats());
        window.SoundGoblin_CacheStats = () => console.table(globals.cacheManager.getAllStats());
    }
    
    if (CONFIG.DEBUG_MODE) console.log('[SoundGoblin] ✓ All enhancements initialized');
    ...
    setTimeout(() => {
        try {
            ...
            if (app && app.classList.contains('hidden') && (!modal || modal.classList.contains('hidden'))) {
                console.warn('[SoundGoblin] App container still hidden after init; forcing visible');
                app.classList.remove('hidden');
                app.style.display = 'block';
            }
        } catch (e) {
            console.warn('[SoundGoblin] Fallback visibility check failed:', e);
        }
    }, 2500);
}
...
function setupKeyboardEvents() {
    ...
    on('soundgoblin:toggle-listening', () => {
...
window.addEventListener('beforeunload', () => {
    if (CONFIG.PERFORMANCE?.ENABLE_METRICS) {
        ...
        if (CONFIG.DEBUG_MODE) {
            ...
            console.log('[SoundGoblin] Final performance report:', report);
        }
    }
